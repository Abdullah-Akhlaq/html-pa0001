...
// Helper function (if not already added)
function createWebSocket(path) {
    try {
        // Check if running on HTTPS
        var isHttps = window.location.protocol === 'https:';
        var wsProtocol = isHttps ? 'wss://' : 'ws://';
        var wsUrl = wsProtocol + 'localhost:7501/' + path;
        
        // For IE compatibility
        if (window.WebSocket) {
            return new WebSocket(wsUrl);
        } else if (window.MozWebSocket) { // For very old Firefox
            return new MozWebSocket(wsUrl);
        } else {
